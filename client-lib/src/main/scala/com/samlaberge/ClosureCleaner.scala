...
object ClosureCleaner extends Logging {
  ...
  // From spark/util/Utils.scala

  private def copyFileStreamNIO(
                         input: FileChannel,
...
//    tryWithSafeFinally {
  ...
  def clean(
    ...
    val m:  Map[Class[_], Set[String]] = Map.empty
    clean(closure, checkSerializable, cleanTransitively, m)
//    logDebug(s"map after clean: $m")
  }
...
//      if (SparkEnv.get != null) {
        // Rewritten by Sam Laberge to remove "SparkEnv" reference
...
//      }
      ...
      case ex: Exception => throw new Exception("Task not serializable")
    }
  }
...
object IndylambdaScalaClosures extends Logging {
...
private class ReturnStatementInClosureException
  extends Exception("Return statements aren't allowed in Spark closures")
    ...
    new MethodVisitor(ASM9) {
      override def visitMethodInsn(
        ...
        if (op == INVOKESPECIAL && name == "<init>" && argTypes.length > 0
          ...
          // Rewritten by Sam Laberge to remove reference to "Utils"
